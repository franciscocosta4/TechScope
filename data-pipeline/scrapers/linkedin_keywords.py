"""Scraper de keywords de descrições de anúncios do LinkedIn.

Percorre a base de dados, abre cada página de anúncio num browser real
(Playwright via CDP), extrai a descrição e guarda as keywords encontradas
na tabela JobKeywords.
"""
...
def scrape_linkedin_description(url: str) -> str | None:
    """Abre a página de um anúncio do LinkedIn e extrai o texto da descrição."""
    if not url:
        return None
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Seletor principal do LinkedIn para a descrição do anúncio.
            desc_elem = soup.select_one("[data-testid='expandable-text-box']")
            if desc_elem:
...
def main():
    with get_connection() as conn:
        ...
        for job in jobs:
            ...
            description = scrape_linkedin_description(external_id)
